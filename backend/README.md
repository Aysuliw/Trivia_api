...
- Returns a list of questions by selected category ,current category ,success value and total number of questions in current category
- Results are paginated in groups of 10.
...
Sample: `curl http://127.0.0.1:5000/questions?page=3 -X POST -H "Content-Type: application/json" -d'{"question":"What is the capital of Finland?", "answer": "Helsinki","difficulty": "3","category": "3"}' `
```
{
  "created": 27,
  "questions": [
...
- returns all questions that matches the search string ,success value and number of questions that matches with search string.
...
- Deletes questions with the given id and return id of the deleted questiona and success value.
...
```
POST/quizzes
- Takes previous questions, quiz category and fetches a random question not in the previous questions
